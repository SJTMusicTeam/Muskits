...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
        ...
        if "${use_xvector}"; then
            ...
            for dset in "${train_set}" "${valid_set}" ${test_sets}; do
                ...
                #   the number of usvs will be different from the original features (raw or fbank).
    ...
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        ...
        for dset in "${train_set}" "${valid_set}"; do
            ...
            # fix_data_dir.sh leaves only usvs which exist in all files
...
if ! "${skip_train}"; then
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.svs_train \
                --collect_stats true \
    ...
    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        ...
        if [ -z "${teacher_dumpdir}" ]; then
            ...
            if [ -e ${_teacher_train_dir}/probs ]; then
                ...
                if [ "${feats_type}" = raw ]; then
                    ...
                    _scp=feats.scp
                    _type=kaldi_ark
                    _odim="$(head -n 1 "${svs_stats_dir}/train/speech_shape" | cut -f 2 -d ",")"
                    _opts+="--odim=${_odim} "
                fi
                _opts+="--train_data_path_and_name_and_type ${_train_dir}/${_scp},speech,${_type} "
                _opts+="--train_shape_file ${svs_stats_dir}/train/speech_shape "
        ...
        log "TTS training started... log: '${svs_exp}/train.log'"
        ...
        ${python} -m espnet2.bin.launch \
            ...
            ${python} -m espnet2.bin.svs_train \
                ...
                --normalize_conf "stats_file=${svs_stats_dir}/train/feats_stats.npz" \
                --resume true \
...
if [ -n "${download_model}" ]; then
    ...
    svs_exp="${expdir}/${download_model}"
    mkdir -p "${svs_exp}"

    # If the model already exists, you can skip downloading
    espnet_model_zoo_download --unpack true "${download_model}" > "${svs_exp}/config.txt"

    # Get the path of each file
    _model_file=$(<"${svs_exp}/config.txt" sed -e "s/.*'model_file': '\([^']*\)'.*$/\1/")
    _train_config=$(<"${svs_exp}/config.txt" sed -e "s/.*'train_config': '\([^']*\)'.*$/\1/")

    # Create symbolic links
    ln -sf "${_model_file}" "${svs_exp}"
    ln -sf "${_train_config}" "${svs_exp}"
...
if ! "${skip_upload}"; then
    if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
        ...
        ${python} -m espnet2.bin.pack svs \
            --train_config "${svs_exp}"/config.yaml \
            --model_file "${svs_exp}"/"${inference_model}" \
            --option "${svs_stats_dir}"/train/feats_stats.npz  \
