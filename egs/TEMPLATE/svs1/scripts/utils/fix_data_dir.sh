...
function filter_utts {
  ...
  if [ -f $data/midi.scp ]; then
    ! cat $data/midi.scp | sort | cmp - $data/midi.scp && \
      echo "midi.scp is not in sorted order (fix this yourself)" && exit 1;
  fi

  if [ -f $data/label ]; then
    ! cat $data/label | sort | cmp - $data/label && \
      echo "label is not in sorted order (fix this yourself)" && exit 1;
  fi
  ...
  for x in feats.scp text segments utt2lang midi.scp label $maybe_wav $maybe_utt2dur $maybe_utt2num_frames; do
    if [ -f $data/$x ]; then
  ...
  for x in utt2spk utt2uniq feats.scp vad.scp text segments utt2lang utt2dur utt2num_frames midi.scp label $maybe_wav $maybe_reco2dur $utt_extra_files; do
    if [ -f $data/$x ]; then
      cp $data/$x $data/.backup/$x
