...
if [ -z "${NO7SINGING}" ]; then
    log "Fill the value of 'NO7SINGING' of db.sh"
    exit 1
fi
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    # The NO7SINGING data should be downloaded from TODO
    # The NO7SINGING mono_label data should be downloaded from https://github.com/mmorise/no7_singing
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    python local/dataset_split.py ${NO7SINGING}/no7singing/wav data/local 0.1 0.1
fi
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    for dataset in ${train_set} ${train_dev} ${recog_set}; do
      ...
      cat ${src_data}/segments | awk '{printf("%s no7singing\n", $1);}' > ${src_data}/utt2spk
